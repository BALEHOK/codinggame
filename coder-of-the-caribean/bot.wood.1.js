...
function kFire(distToTarget, targetSpeed) {
  return distToTarget > 10
    ? -1
    : (100 - 50 * targetSpeed) / (distToTarget * distToTarget * (1 + 2*targetSpeed));
}
function step() {
  ++stepNum;
  ...
  for (let i = 0; i !== myShips.length; i++) {
    ...
    if (stepNum - s.fireStep > fireDelay) {
      for (let j = 0; j !== fowShips.length; j++) {
        const f = fowShips[j];
        const dist = getDistance(s, f);

        const steps = Math.ceil((1 + dist) / 3);


        actions.push(new Fire(kFire(dist, f.speed), f.x, f.y));
      }
    }

    // function getShipPos(ship, steps) {
...
- обходить мины
- стрелять если стоит на месте
- стрелять на опережение (на один, максимум два, хода)
